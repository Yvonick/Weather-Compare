...
- Temperature uses Combined: average lines and min–max bands (within-period extrema, not forecast uncertainty). The Separate option is hidden for now, including for older saved settings and share links. One or two visible locations show all bands; larger comparisons show the previewed range or up to two locked ranges. Clicking, tapping, Enter, or Space toggles a location's lock; selecting a third never silently replaces the chosen pair. Hovering any data point updates the compact readout independently of locked ranges. With the chart focused, Left/Right inspect dates, Up/Down inspect locations, and Home/End jump to endpoints; no date selector is needed. Inspection and lock state are local to the chart, not shared or saved. Popouts preserve locks through zoom. Missing extrema leave gaps rather than being inferred from a single sample. Tables and exports retain all measures. Typography matches the UV Exposure tool's Segoe UI / sans-serif stack.
