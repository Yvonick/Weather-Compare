...
- Temperature defaults to Separate: Minimum, Average, and Maximum in three aligned charts with a shared scale. Combined shows average lines and min–max bands (within-period extrema, not forecast uncertainty). One or two visible locations show all bands; larger comparisons show only the selected location's band. Hover or keyboard focus previews a range; clicking, tapping, Enter, or Space pins it. The date/period control and readout expose exact values without requiring hover. The layout is saved and shared; transient inspection state is not. Both layouts support popouts. Missing extrema leave gaps rather than being inferred from a single sample. Tables and exports retain all measures.
