...
- Temperature defaults to Combined: average lines and min–max bands (within-period extrema, not forecast uncertainty). Separate shows Minimum, Average, and Maximum in three aligned charts with a shared scale. One or two visible locations show all bands; larger comparisons show only the priority location's band. Hover or keyboard focus previews a range while unlocked; clicking, tapping, Enter, or Space locks the priority location against hover/focus changes. Click the locked location again to unlock, or another location to transfer the lock. A visible badge explains how to unlock. The date/period control and compact readout expose exact values without covering the graph or requiring hover. The layout is saved and shared (existing explicit Separate preferences are respected); transient inspection/lock state is not. Both layouts support popouts, preserving the lock through zoom. Missing extrema leave gaps rather than being inferred from a single sample. Tables and exports retain all measures.
